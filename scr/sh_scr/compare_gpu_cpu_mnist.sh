...
for i in $(seq 0 4); do
    rm -rf data/compare_gpu_cpu_mnist/
    mkdir -p data/compare_gpu_cpu_mnist/

    echo "Dataset with Nd: 500000 and Dim: ${Dim[$i]}"
    python scr/data_scr/mnist_generation.py -sz 12000 -dim ${Dim[$i]} -dir compare_gpu_cpu_mnist
